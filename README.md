...
## 接口文档
根据商家id查询食物信息
● URL：http://47.101.196.82:8988/elm/food/list/110110
● Method：GET
● 需要登录：
● 需要鉴权：否
请求参数
...
条件：请求参数合法。
状态码：200 OK
响应示例：响应会将查询到信息数据返回，一个businessId为 110110的商家下面的食品将会返回：
{
    "success": true,
    "code": 1,
    "message": "请求成功",
...
条件：请求参数合法。
状态码：200 OK
响应示例：响应会将查询到的食物数据返回，一个foodid为 20220070204的食物的数据：
{
    "success": true,
    "code": 1,
    "message": "请求成功",
...
● URL：http://47.101.196.82:8988/elm/foodBusinessClassify/getFoodClassifyId/1001
● Method：GET
● 需要登录：
● 需要鉴权：否
...
条件：请求参数合法。
状态码：200 OK
响应示例：响应会将查询到食物分类的id数据返回，一个foodClassifyId为 1001的食物分类信息返回：
{
    "success": true,
    "code": 1,
    "message": "请求成功",
...
根据商家id查询食物的分类信息
如：一个商家有多个食物分类
● URL：http://47.101.196.82:8988/elm/foodClassify/getBusinessId/110113
● Method：GET
● 需要登录：
● 需要鉴权：否
...
条件：请求参数合法。
状态码：200 OK
响应示例：响应会将查询到的食物分类的id信息数据返回，一个businessId为 110113的商家下的食物分类将会返回：
{
    "success": true,
    "code": 1,
    "message": "请求成功",
...
通过userId查询购物车信息
● URL：http://47.101.196.82:8988/elm/cart/getUserId/18046873682
● Method：get
请求参数
参数	类型	约束
userId	Long	无
...
添加购物车
● URL：http://47.101.196.82:8988/elm/cart/saveCart
● Method：post
请求参数
参数	类型	约束
cart	int	
...
通过userId来删除购物车
● URL：http://47.101.196.82:8988/elm/cart/delete/{userId}
● Method：delete
请求参数
参数	类型	约束
userId	Long	
...
条件：请求参数合法。
状态码：200 OK
响应示例：响应会将查询的评论信息数据返回，根据userId查询到的数据进行返回：
...
条件：请求参数合法。
状态码：200 OK
响应示例：响应会将查询到的用户信息数据返回，一个userId为 7190764205的用户信息将会返回：
{
    "success": true,
    "code": 1,
    "message": "请求成功",
...
💡 之后的接口都是拦截器拦截的请求，需要携带token
💡 eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VySWQiOjE3MzcwNzA5MTYwLCJ1c2VybmFtZSI6IuWRqOaUv-S8nyIsImlhdCI6MTY2MjU3MTA4MSwiZXhwIjoxNjYyNjU3NDgxLCJqdGkiOiJiY2U4NGVmNjU4MmM0OTkzYjllYzAzZjRmODhkMjNhYyJ9.9IPibjJbbfPSIC5L0SZHvtH0rsSG9dwaxtpkqS1oZQg
...
请求示例
这个是老师代码中生成订单的方法涉及三个以上的请求参数，直接绑定我的这个接口名就可以，不是json传递
